...
CTBotInlineKeyboard DiKbd;  // custom inline keyboard object helper
...
void call_status (TBMessage msg) {
  ...
  status =  String("Temp: ") + String(climate.temp) + \
            ...
            String("\nDiCtrl: ") + String(di) + \
            String("\nTimer: ") + String((float)timer/60) + String(" H");
  myBot.sendMessage(msg.sender.id,  status, CmdKbd);
}

void call_timer(TBMessage msg, int _di) {
    di = (float)_di; 
  ...
  if (myBot.getNewMessage(msg)) {
    ...
    if (msg.messageType == CTBotMessageText) {
      ...
      if (msg.callbackQueryData.equals(ON_CALLBACK)) {
        myBot.sendMessage(msg.sender.id, "Choose DI", DiKbd);
      } else if (msg.callbackQueryData.equals("DI_70")) {
        call_timer(msg, 70);
...
void setup() {
  ...
  //
  DiKbd.addButton("70", "DI_70", CTBotKeyboardButtonQuery);
  DiKbd.addButton("71", "DI_71", CTBotKeyboardButtonQuery);
  DiKbd.addButton("72", "DI_72", CTBotKeyboardButtonQuery);
  DiKbd.addButton("73", "DI_73", CTBotKeyboardButtonQuery);
  DiKbd.addRow();
  DiKbd.addButton("74", "DI_74", CTBotKeyboardButtonQuery);
  DiKbd.addButton("75", "DI_75", CTBotKeyboardButtonQuery);
  DiKbd.addButton("76", "DI_76", CTBotKeyboardButtonQuery);
  DiKbd.addButton("77", "DI_77", CTBotKeyboardButtonQuery);
  //
